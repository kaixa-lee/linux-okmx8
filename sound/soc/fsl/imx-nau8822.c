...
struct imx_nau8822_data {
	...
	struct snd_soc_dai_link imx_nau8822_dai[3];
};

static int hp_jack_status_check(void *data)
{
	struct imx_nau8822_data *imx_data = (struct imx_nau8822_data *)data;
...
SND_SOC_DAILINK_DEFS(hifi,
	...
	DAILINK_COMP_ARRAY(COMP_CODEC(NULL, "nau8822-hifi")),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
...
static int imx_nau8822_probe(struct platform_device *pdev)
{
	...
	data->imx_nau8822_dai[0].name = "HiFi";
	data->imx_nau8822_dai[0].stream_name = "HiFi";
	data->imx_nau8822_dai[0].ops = &imx_hifi_ops;
	data->imx_nau8822_dai[0].cpus = hifi_cpus;
	data->imx_nau8822_dai[0].num_cpus = ARRAY_SIZE(hifi_cpus);
	data->imx_nau8822_dai[0].codecs = hifi_codecs;
	data->imx_nau8822_dai[0].num_codecs = ARRAY_SIZE(hifi_codecs);
	data->imx_nau8822_dai[0].platforms = hifi_platforms;
	data->imx_nau8822_dai[0].num_platforms = ARRAY_SIZE(hifi_platforms);

	data->imx_nau8822_dai[1].name = "HiFi-ASRC-FE";
	data->imx_nau8822_dai[1].stream_name = "HiFi-ASRC-FE";
	data->imx_nau8822_dai[1].dynamic = 1;
	data->imx_nau8822_dai[1].ignore_pmdown_time = 1;
	data->imx_nau8822_dai[1].dpcm_playback = 1;
	data->imx_nau8822_dai[1].dpcm_capture = 1;
	data->imx_nau8822_dai[1].dpcm_merged_chan = 1;
	data->imx_nau8822_dai[1].cpus = hifi_fe_cpus;
	data->imx_nau8822_dai[1].num_cpus = ARRAY_SIZE(hifi_fe_cpus);
	data->imx_nau8822_dai[1].codecs = hifi_fe_codecs;
	data->imx_nau8822_dai[1].num_codecs = ARRAY_SIZE(hifi_fe_codecs);
	data->imx_nau8822_dai[1].platforms = hifi_fe_platforms;
	data->imx_nau8822_dai[1].num_platforms = ARRAY_SIZE(hifi_fe_platforms);

	data->imx_nau8822_dai[2].name = "HiFi-ASRC-BE";
	data->imx_nau8822_dai[2].stream_name = "HiFi-ASRC-BE";
	data->imx_nau8822_dai[2].no_pcm = 1;
	data->imx_nau8822_dai[2].ignore_pmdown_time = 1;
	data->imx_nau8822_dai[2].dpcm_playback = 1;
	data->imx_nau8822_dai[2].dpcm_capture = 1;
	data->imx_nau8822_dai[2].ops = &imx_hifi_ops;
	data->imx_nau8822_dai[2].be_hw_params_fixup = be_hw_params_fixup;
	data->imx_nau8822_dai[2].cpus = hifi_be_cpus;
	data->imx_nau8822_dai[2].num_cpus = ARRAY_SIZE(hifi_be_cpus);
	data->imx_nau8822_dai[2].codecs = hifi_be_codecs;
	data->imx_nau8822_dai[2].num_codecs = ARRAY_SIZE(hifi_be_codecs);
	data->imx_nau8822_dai[2].platforms = hifi_be_platforms;
	data->imx_nau8822_dai[2].num_platforms = ARRAY_SIZE(hifi_be_platforms);

	if (data->is_capture_only) {
		data->imx_nau8822_dai[0].capture_only = true;
		data->imx_nau8822_dai[1].capture_only = true;
		data->imx_nau8822_dai[2].capture_only = true;
	}

	if (data->is_playback_only) {
		data->imx_nau8822_dai[0].playback_only = true;
		data->imx_nau8822_dai[1].playback_only = true;
		data->imx_nau8822_dai[2].playback_only = true;
	}
	...
	data->card.dai_link = data->imx_nau8822_dai;

	if (data->is_codec_rpmsg) {
		data->imx_nau8822_dai[0].codecs->name     = "rpmsg-audio-codec-nau8822";
		data->imx_nau8822_dai[0].codecs->dai_name = "rpmsg-nau8822-hifi";
	} else
		data->imx_nau8822_dai[0].codecs->of_node	= codec_np;

	data->imx_nau8822_dai[0].cpus->dai_name = dev_name(&cpu_pdev->dev);
	data->imx_nau8822_dai[0].platforms->of_node = cpu_np;

	if (!asrc_pdev) {
		data->card.num_links = 1;
	} else {
		data->imx_nau8822_dai[1].cpus->of_node = asrc_np;
		data->imx_nau8822_dai[1].platforms->of_node = asrc_np;
		if (data->is_codec_rpmsg) {
			data->imx_nau8822_dai[2].codecs->name     = "rpmsg-audio-codec-nau8822";
			data->imx_nau8822_dai[2].codecs->dai_name = "rpmsg-nau8822-hifi";
		} else
			data->imx_nau8822_dai[2].codecs->of_node	= codec_np;
		data->imx_nau8822_dai[2].cpus->dai_name = dev_name(&cpu_pdev->dev);
	...
	if (gpio_is_valid(data->imx_hp_jack_gpio.gpio)) {
		...
		ret = imx_nau8822_jack_init(&data->card, &data->imx_hp_jack,
	...
	if (gpio_is_valid(data->imx_mic_jack_gpio.gpio)) {
		if (!data->is_headset_jack) {
			...
			ret = imx_nau8822_jack_init(&data->card, &data->imx_mic_jack,
...
static const struct of_device_id imx_nau8822_dt_ids[] = {
	...
	{ .compatible = "fsl,imx7d-evk-nau8822"  },
	{ /* sentinel */ }
};
