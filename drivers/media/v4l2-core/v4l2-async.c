...
static int waiting_list_igonre;
/*
...
v4l2_async_notifier_can_complete(struct v4l2_async_notifier *notifier)
{
	...
	if (!list_empty(&notifier->waiting) && waiting_list_igonre == 0)
		return false;
...
int v4l2_async_register_subdev(struct v4l2_subdev *sd)
{
	...
	list_for_each_entry(notifier, &notifier_list, list) {
		...
		if (ret)
			goto err_unbind;
		if(!strncmp("basler-camera-vvcam ", sd->name, 18))
			waiting_list_igonre = 1;
		ret = v4l2_async_notifier_try_complete(notifier);
		waiting_list_igonre = 0;
		if (ret)
			goto err_unbind;
