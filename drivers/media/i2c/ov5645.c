...
struct ov5645 {
	...
	struct mutex lock; /* lock to protect power state */
	int power_count;
...
static int ov5645_set_power_on(struct ov5645 *ov5645)
{
	...
	gpiod_set_value_cansleep(ov5645->enable_gpio, 0);	// LOW
	gpiod_set_value_cansleep(ov5645->rst_gpio, 0);		// HIGH

	gpiod_set_value_cansleep(ov5645->enable_gpio, 1);	// HIGH
	usleep_range(5000, 15000);

	gpiod_set_value_cansleep(ov5645->rst_gpio, 1);		//LOW
	msleep(50);
	gpiod_set_value_cansleep(ov5645->rst_gpio, 0);		//HIGH
	msleep(5);
				 ...
				 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->pad != 0)
		return -EINVAL;
	if (code->index >= ARRAY_SIZE(ov5645_formats))
		return -EINVAL;

	code->code = ov5645_formats[code->index].code;
			     ...
			     struct v4l2_subdev_format *format)
{
	...
	sensor->fmt.width = mbus_fmt->width;
	sensor->fmt.height = mbus_fmt->height;
	sensor->fmt.code = mbus_fmt->code;
	sensor->fmt.field = mbus_fmt->field;
	sensor->fmt.colorspace = mbus_fmt->colorspace;
	out:
...
static int ov5645_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	if (enable) {
		...
		printk("%s width:%d height:%d\n",
				__func__, ov5645->fmt.width, ov5645->fmt.height);
		for (i = 0; i <  ARRAY_SIZE(ov5645_mode_info_data); i++) {
			if (ov5645_mode_info_data[i].width == ov5645->fmt.width) {
				ov5645->current_mode = &ov5645_mode_info_data[i];
				break;	
			}
		}
		if (i >= ARRAY_SIZE(ov5645_mode_info_data)) {
			return -EINVAL;
		}
...
static int ov5645_link_setup(struct media_entity *entity, const struct media_pad *local, const struct media_pad *remote, unsigned int flags)
{
	return 0;
}

static const struct media_entity_operations ov5645_subdev_entity_ops = {
...
static int ov5645_probe(struct i2c_client *client)
{
	...
	ov5645->enable_gpio = devm_gpiod_get(dev, "enable", GPIOD_OUT_LOW); //LOW
	if (IS_ERR(ov5645->enable_gpio)) {
		dev_err(dev, "cannot get enable gpio\n");
		return PTR_ERR(ov5645->enable_gpio);
	}

	ov5645->rst_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_LOW);	//HIGH
	if (IS_ERR(ov5645->rst_gpio)) {
		dev_err(dev, "cannot get reset gpio\n");
		return PTR_ERR(ov5645->rst_gpio);
	}

	mutex_init(&ov5645->lock);
...
#if 0
	ov5645->link_freq = v4l2_ctrl_new_std_menu(&ov5645->ctrls, 
			&ov5645_ctrl_ops, 
	...
	ov5645->sd.flags |= V4L2_SUBDEV_FL_HAS_EVENTS | V4L2_SUBDEV_FL_HAS_DEVNODE;
	ov5645->pad.flags = MEDIA_PAD_FL_SOURCE;
	ov5645->sd.dev = &client->dev;
	ov5645->sd.entity.function = MEDIA_ENT_F_CAM_SENSOR;
	ov5645->sd.entity.ops = &ov5645_subdev_entity_ops;
